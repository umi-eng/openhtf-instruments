# Siglent SDM3065X OpenHTF plug

This package provides an OpenHTF plug for taking single measurements from the
Siglent SDM3065X over PyVISA. It supports DC/AC voltage and current, 2-wire and
...
`SIGLENT_SDM3065X_RESOURCE`. For LAN, a common resource form is
...
os.environ["SIGLENT_SDM3065X_RESOURCE"] = "TCPIP0::192.0.2.10::inst0::INSTR"


@plugs.plug(multimeter=SiglentSDM3065XPlug)
...
`measure()` accepts a `MeasurementFunction` or its name as a string. An optional `range` is supported for voltage, current, resistance, and
capacitance. Temperature readings can specify `temperature_transducer` (`RTD`
